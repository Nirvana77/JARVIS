...
| Orchestrator | `jarvis/core/orchestrator.py` | single asyncio loop; blocking work via `asyncio.to_thread`; injected components; after a command it keeps listening `capture.follow_up_s` (default 10s) for a follow-up with no wake word, then speaks the standby line. **Voice barge-in**: while a command is being transcribed a second listen runs; a fresh full sentence (≥ `capture.barge_in_min_speech_s`, noise-filtered) abandons that transcription and becomes the new command — no wake word. The abandoned whisper decode is told to stop after its current segment (CTranslate2 can't be killed mid-segment) and its result is dropped. Also **press Enter** (TTY) to cancel. Barge-in does not run during TTS playback (echo — M4). |
